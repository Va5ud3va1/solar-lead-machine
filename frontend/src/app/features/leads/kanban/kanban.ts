...
export class KanbanComponent implements OnInit, OnDestroy {
  ...
  loadLeads(): void {
    ...
    this.leadService.filterLeads(this.searchText, this.selectedStatus).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data: Lead[]) => { this.leads = data; this.sortLeads(); this.initializeColumns(); this.updateMetrics(); this.updateChartData(); this.loading = false; this.cdr.detectChanges(); },
  ...
  drop(event: CdkDragDrop<Lead[]>, newStatus: LeadStatus): void {
    ...
    this.leadService.updateLeadStatus(lead.id, newStatus).subscribe({
      next: (result) => { this.leads = this.leads.map(l => l.id === result.id ? result : l); this.distributeLeadsToColumns(); this.updateColumnCounts(); this.updateMetrics(); this.updateChartData(); this.updating = false; this.toastService.success(`Lead "${result.customer}" moved to ${newStatus}`); this.cdr.detectChanges(); },
  ...
  deleteLead(): void {
    ...
    this.leadService.deleteLead(this.leadToDelete.id).subscribe({
      next: () => { this.leads = this.leads.filter(l => l.id !== this.leadToDelete!.id); this.distributeLeadsToColumns(); this.updateColumnCounts(); this.updateMetrics(); this.updateChartData(); this.cancelDelete(); this.toastService.success(`Lead "${deletedLeadName}" deleted successfully`); },
